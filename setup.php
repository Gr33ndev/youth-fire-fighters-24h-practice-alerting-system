...
<div class="w-2/3 bg-white rounded-xl shadow-lg p-6 m-8">
    ...
    if ($numberOfOperations == 0 && empty($_POST)) {
        ...
        <form action="setup.php" method="get" class="space-y-4">
            <div>
                ...
                <input type="number" id="numberOfOperations" name="numberOfOperations"
    ...
    if ($numberOfOperations != 0 && empty($_POST)) {
        ...
        <form action="setup.php" method="POST" class="space-y-4 divide-y">
            ...
            for ($i = 0; $i < $numberOfOperations; $i++) {
                ...
                <div class="space-y-4 py-2">
                    ...
                    <div>
                        ...
                        <input type="text" id="operationDescription"
                               name="operationDescription<?php echo "-" . ($i + 1); ?>"
                               value="<?php echo $operationDescription[$i + 1] ?? ""; ?>"
                               class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    <div>
                        <label for="address" class="block text-sm font-medium text-gray-700">Adresse</label>
                        <input type="text" id="address" name="address<?php echo "-" . ($i + 1); ?>"
                               value="<?php echo $address[$i + 1] ?? ""; ?>"
                               class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    <div>
                        ...
                        <input type="date" id="operationDate" name="operationDate<?php echo "-" . ($i + 1); ?>"
                               value="<?php echo $operationDate[$i + 1] ?? ""; ?>"
                               class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                    <div>
                        <label for="operationTime"
                               class="block text-sm font-medium text-gray-700">Einsatzuhrzeit</label>
                        <input type="time" id="operationTime" name="operationTime<?php echo "-" . ($i + 1); ?>"
                               value="<?php echo $operationTime[$i + 1] ?? ""; ?>"
                               class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
            ...
            <button type="submit"
                    class="flex justify-center py-2 px-4 mb-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mx-auto">
                Speichern
            </button>
    ...
    ?>
    <a class="flex justify-center text-xs mt-4 text-gray-500"
       href="third-party-license-notices/third-party-license-notices.html">Third Party License Notice</a>
</body>
</html>
